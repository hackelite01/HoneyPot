...
from .Honeypot_Project import main
...
server = None
t2 = None
# LOG_FILE_PATH = './honeypot/Honeypot_Project/var/web_honeypot.log'
...
def dashboard(request):
    ip_addr=[]
    try:
        logs_web = handle_logs('./honeypot/Honeypot_Project/var/web_honeypot.log')
        for log in logs_web:
            if log['ip_addr'] not in ip_addr:
                ip_addr.append(log['ip_addr'])
        logs_net = handle_logs('./honeypot/Honeypot_Project/var/net_honeypot.log')
        for log in logs_net:
            try:
                ...
                pass
        logs_key = handle_logs('./honeypot/Honeypot_Project/var/key_logger.log')
        for log in logs_key:
            if log['ip_addr'] not in ip_addr:
                ip_addr.append(log['ip_addr'])
    ...
    if request.method == 'POST':
        if flask_thread is None or not flask_thread.is_alive():
            def run_flask():
                ...
                flask_server = make_server('127.0.0.1', 5000, main.WebsiteTrap.app, threaded=True)
                flask_server.serve_forever()

            flask_thread = threading.Thread(target=run_flask)
            flask_thread.start()
            return JsonResponse({'status': 'started', 'ip': '127.0.0.1', 'port': '5000'})
        else:
...
def file_analysis(request):
    try:
        key_logs = handle_logs('./honeypot/Honeypot_Project/var/file_analysis.log')
        keys=[]
...
def Keylogger(request):
    try:
        key_logs = handle_logs('./honeypot/Honeypot_Project/var/key_logger.log')
        # print(key_logs)
        keys=[]
        for key_log in key_logs:
            for key in key_log.keys():
...
def network(request):
    try:
        key_logs = handle_logs('./honeypot/Honeypot_Project/var/net_honeypot.log')
        keys=[]
        for key_log in key_logs:
            for key in key_log.keys():
                if key not in keys:
...
def photo(request):
    try:
        key_logs = handle_logs('./honeypot/Honeypot_Project/var/photo_metadata.log')
        keys=[]
        for key_log in key_logs:
            for key in key_log.keys():
                if key not in keys:
...
def website(request):
    try:
        key_logs = handle_logs('./honeypot/Honeypot_Project/var/web_honeypot.log')
        keys=[]
        for key_log in key_logs:
            for key in key_log.keys():
                if key not in keys:
